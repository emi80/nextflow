...
class FilePorter {
    ...
    FilePorter() {
        this.stagingDir = getDownloadPath()
    }
    ...
    Map<String,Path> stageForeignFiles(Map<String, Path> filesMap) {
        ...
        def futures = new ArrayList<Future<NamePathPair>>(actions.size())
        while ( futures.size() < actions.size() ) {
            def future = stageExecutor.poll(pollTimeout, pollTimeoutUnit)
            if ( !future ) {
    ...
    private String getDownloadMessage(List<Path> filePaths) {
     ...
     *      This value can be defined by using the {@code filePorter.downloadPath} configuration setting (default: <session_workDir/tmp>).
     */
    static Path getDownloadPath() {
        String result = Global.session.config.navigate('filePorter.downloadPath') as String
        if( !result )
            result = System.getenv().get('NXF_DOWNLOAD_PATH')
        if( !result )
            result = Global.session.workDir.resolve("download").toString()

        log.debug "filePorter.downloadPath=$result"
    ...
    static synchronized private CompletionService createExecutor(Session session) {
        ...
        final svc = new ExecutorCompletionService<>(result)
